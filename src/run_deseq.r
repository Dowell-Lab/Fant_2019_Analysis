...
correct_seqdata <- read.delim("counts_fullgene.txt_without_header", stringsAsFactors = FALSE, header = TRUE,
                              row.names = 1)
...
## We have to fix the counts table by removing the first row, hence "counts_fix.txt"
full_seqdata <- read.delim("counts_initiation_fix.txt", stringsAsFactors = FALSE, header = TRUE,
                           row.names = 1)
## Then, we filter to only include the resolved isoforms
...
png(paste0(prefix, "-diffexpr-hist.png"), 1500, 1000, pointsize = ptsize)
hist(res$pvalue, breaks = 50, col = "grey")
dev.off()

## png('MA_plot.png') plotMA(res) dev.off()

png(paste0(prefix, "-diffexpr-maplot.png"), 1500, 1000, pointsize = ptsize)
