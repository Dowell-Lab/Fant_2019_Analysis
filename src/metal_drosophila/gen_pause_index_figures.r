...
setwd("/home/zach/dowell_lab/pausing_meta_analysis/out/metal_drosophila/pausing")

## You only need to adjust the filenames here...
c_1_301 <- read_delim("LacI_plus_Cu1_S1_R1_001.bedGraph_pause_ratios_PROD.data",
                      col_names=c('tx_name', 'strand', 'c_1_301', 'coverage_c1_301'), delim=" ") %>%
    subset(select=-strand)
c_2_301 <- read_delim("LacI_plus_Cu2_S2_R1_001.bedGraph_pause_ratios_PROD.data",
                      col_names=c('tx_name', 'strand', 'c_2_301', 'coverage_c2_301'), delim=" ") %>%
    subset(select=-strand)
c_3_301 <- read_delim("LacI_plus_Cu3_S3_R1_001.bedGraph_pause_ratios_PROD.data",
                      col_names=c('tx_name', 'strand', 'c_3_301', 'coverage_c3_301'), delim=" ") %>%
    subset(select=-strand)
p_1_301 <- read_delim("TAF1_plus_Cu2_S5_R1_001.bedGraph_pause_ratios_PROD.data",
                      col_names=c('tx_name', 'strand', 'p_1_301', 'coverage_p1_301'), delim=" ") %>%
    subset(select=-strand)
p_2_301 <- read_delim("TAF1_plus_Cu3_S6_R1_001.bedGraph_pause_ratios_PROD.data",
                      col_names=c('tx_name', 'strand', 'p_2_301', 'coverage_p2_301'), delim=" ") %>%
    subset(select=-strand)
p_3_301 <- read_delim("TAF1_plus_Cu_S4_R1_001.bedGraph_pause_ratios_PROD.data",
...
gaga_genes <- read_delim("dm6_matched_genes_NVNVMGNRMR.data", delim="\t",
                         col_names = c("name", "seq"))
inr_genes <- read_delim("dm6_matched_genes_BBCABW.data", delim="\t",
                        col_names = c("name", "seq"))
tata_genes <- read_delim("dm6_matched_genes_WWWW.data", delim="\t",
                         col_names = c("name", "seq"))
mte_genes <- read_delim("dm6_matched_genes_CGANC....CGG.data", delim="\t",
                        col_names = c("name", "seq"))
dpe_genes <- read_delim("dm6_matched_genes_RGWYVT.data", delim="\t",
                        col_names = c("name", "seq"))
pb_genes <- read_delim("dm6_matched_genes_KCGRWCG.data", delim="\t",
                       col_names = c("name", "seq"))
