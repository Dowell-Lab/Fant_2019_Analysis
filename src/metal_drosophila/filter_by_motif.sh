...
DirPrefix=/scratch/Users/zama8258/taf1_drosophila_pro_seq_metal/output/pausing
...
OutFile="$DirPrefix"/"$name"_matched_genes_"$motif".data
...
grep -i	"$motifpattern" "$Sequences" | awk '!_[$1]++' > "$MatchedSequences" &
wait

cat	"$MatchedSequences" | sed "s/(+)//g; s/(-)//g" > "$OutFile"
