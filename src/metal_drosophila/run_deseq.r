...
setwd('/home/zach/dowell_lab/pausing_meta_analysis/out/metal_drosophila/deseq')
num_reps <- 3
...
condition <- factor(#c(rep("treated", num_reps), rep("untreated", num_reps)),
    c("untreated", "untreated", "treated", "untreated", "treated", "treated"),
...
## We have to fix the counts table by removing the first row, hence "counts_fix.txt"
initiation_seqdata <- read.delim("counts_initiaton.txt_without_header", stringsAsFactors = FALSE, header = TRUE,
                                 row.names = 1)
## Then, we filter to only include the resolved isoforms
