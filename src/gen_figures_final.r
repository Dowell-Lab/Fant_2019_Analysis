...
ddt_PROD$c_coverage_mean <- rowMeans(ddt_PROD[c('coverage_c1_PROD', 'coverage_c2_PROD')], na.rm=FALSE)
ddt_PROD$p_coverage_mean <- rowMeans(ddt_PROD[c('coverage_p1_PROD', 'coverage_p2_PROD')], na.rm=FALSE)
...
ddt_PROD$coverage_mean <- rowMeans(ddt_PROD[c('c_coverage_mean', 'p_coverage_mean')], na.rm=TRUE)
...
ggsave(
    str_c("/scratch/Users/zama8258/pause_output/prepublish_scatter.png"),
    plot = last_plot(), device = "png")

## ecdf plot
ggplot(data = ddt_PROD) +
    ...
    scale_x_log10()
ggsave(
    str_c("/scratch/Users/zama8258/pause_output/prepublish_ecdf.png"),
    plot = last_plot(), device = "png")

fddt_PROD <- filter(ddt_PROD, pause_diff > -6e-5)
...
ggplot(data = ddt_PROD, mapping=aes(x=coverage_mean, y=pause_diff), alpha=1/10) +
    ...
    ## scale_y_log10()
    ggsave(
        str_c("/scratch/Users/zama8258/pause_output/prepublish_MA.png"),
        plot = last_plot(), device = "png")
...
ggplot(data = ddt_PROD_wellexpressed, mapping=aes(x=p_pause_mean, y=c_pause_mean), alpha=1/10) +
    geom_bin2d(bins=75) + geom_abline(aes(intercept=0,slope=1)) +
    theme_tufte() +
    scale_fill_gsea() +
    labs(x = "Control",
    ...
    scale_x_log10() + scale_y_log10()
ggsave(
    str_c("/scratch/Users/zama8258/pause_output/prepublish_wellexpressed_scatter.png"),
    plot = last_plot(), device = "png")

## PROD_wellexpressed ecdf
ggplot(data = ddt_PROD_wellexpressed) +
    stat_ecdf(aes(p_pause_mean, color='Control')) + stat_ecdf(aes(c_pause_mean, color='Treatment')) +
    theme_tufte() +
    scale_color_npg() +
    theme(legend.title=element_blank()) +
    ...
    scale_x_log10()
ggsave(
    str_c("/scratch/Users/zama8258/pause_output/prepublish_wellexpressed_ecdf.png"),
    plot = last_plot(), device = "png")

fddt_PROD_wellexpressed <- filter(ddt_PROD_wellexpressed, pause_diff > -6e-5)

## MA-ish plot
ggplot(data = ddt_PROD_wellexpressed, mapping=aes(x=coverage_mean, y=pause_diff), alpha=1/10) +
    geom_bin2d(bins=75) + geom_hline(yintercept=0) +
    scale_fill_gsea() +
    theme_tufte() +
    labs(x = "Normalized Coverage Level",
    ...
    ## scale_y_log10()
    ggsave(
        str_c("/scratch/Users/zama8258/pause_output/prepublish_wellexpressed_MA.png"),
        plot = last_plot(), device = "png")
